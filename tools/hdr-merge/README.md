...
| `--log-ceiling` | `0.92` | Wohin die hellste Stelle gelegt wird. Die 15 % darüber sind **freie Reserve** für die spätere Belichtungskorrektur — der Kern der ganzen Betriebsart. |
| `--profile` | `fertig` | `fertig` liefert ein **entwickeltes** Bild: Log-Kodierung und die dazu passende Wiedergabe in einem Schritt. `log` liefert nur die flache Kodierung — als Vorlage für eine eigene LUT; sie sieht ohne Gegenkurve milchig aus, und das ist Absicht. `bild` ist der alte Weg. |
| `--black-point` | `0.03` | Wohin die dunkelste Stelle gelegt wird. **Ohne diesen Schritt bleibt ein Schleier über dem Bild** — eine Innenaufnahme enthält von sich aus selten etwas wirklich Schwarzes. Gemessen: ohne 0,19–0,26, mit 0,030–0,159; der kommerzielle Dienst liegt bei 0,020–0,101. Der Wert wird **am Bild** gesetzt, nicht als fester Abzug, weil die dunkelste Stelle je Raum verschieden ist. |
| `--mid-point` | `0.68` | Wohin das Raumniveau in der Wiedergabe gelegt wird. **Nicht** der Hebel für die Gesamthelligkeit – wird er hochgezogen, steigen die Fenster mit und landen im obersten Bereich. Dafür ist der Weißpunkt zuständig (siehe unten). |
| *(fest)* `WEISS_PERZENTIL` / `WEISS_ZIEL` | `85` / `0.78` | Der Weißpunkt wird an den hellen **Innenflächen** verankert, nicht am Bildmaximum. Am Dienst gemessen: p85 liegt bei 0,755–0,808, p99,5 bei 0,915–0,918 — der Abstand von weißen Wänden zu Fenstern beträgt also nur rund 0,12. Fenster nehmen üblich unter 15 % der Fläche ein und liegen damit oberhalb von p85; das trennt sie **ohne Fenstererkennung**, nur über ein Perzentil. |
| `--log-slope` | `0.058` | **Anteil des Wertebereichs je Blendenstufe.** Fest, unabhängig vom Bildinhalt — das ist der Kern. Vorher wurde der Umfang *dieser* Szene auf das Band gedehnt, wodurch jedes Bild eine andere Kennlinie bekam (gemessen 0,0557–0,0622 je Blende, Raumniveau zwischen 0,474 und 0,568). Ein Preset bedeutete dann bei jedem Bild etwas anderes. S-Log3 macht es richtig: feste Steigung, fester Anker. Größer = kontrastreicher, aber weniger Luft über den Fenstern. |
| `--log-color` | `1.1` | Nimmt die **Farbstauchung** zurück, die die Kennlinie unvermeidlich erzeugt. Bei dreizehn Blendenstufen auf ein Band von 0,8 schrumpft jeder Farbunterschied auf ein Sechzehntel — das Bild wirkt entsättigt, und ein Sättigungsregler in Lightroom holt es *nicht* zurück, weil er die Werte als fertiges sRGB behandelt und nichts von der Stauchung weiß. Gemessen an zwei Szenen gegen dieselbe Szene normal entwickelt: Faktor 1,0 → 0,048/0,042 (Szene: 0,111/0,078), Faktor 2,2 → 0,106/0,092. Die Entsättigung der *Lichter* bleibt erhalten, sie stammt aus der kanalweisen Kennlinie. |
| `--log-toe` | `0.004` | Linearer Fuß der Kennlinie. Ohne ihn läuft der Logarithmus in den Tiefen ins Bodenlose und hebt dort — weil er kanalweise wirkt — den jeweils schwächsten Kanal am stärksten an: sichtbar als Farbstich in dunklen Flächen. |
